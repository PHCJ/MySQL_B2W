...
CREATE TABLE base_unica AS SELECT financeiro.*, vendascol, visitascol FROM
    financeiro,
    vendas,
    visitas
WHERE
    (idVendas = idVisitas) && (idFinanceiro = idVendas);

-- ---------------------------------------------------------------------------------------------------
-- Procedimento para Somar o faturamento
-- ---------------------------------------------------------------------------------------------------
DELIMITER $$
...
END $$
DELIMITER ;
-- chamando o procedimento que retornará o valor do faturamento de cada departamento.
CALL departamento_maior_faturamento;
-- ---------------------------------------------------------------------------------------------------
-- Procedimento para calcular o departamento com maior lucro.
-- ---------------------------------------------------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE departamento_maior_lucro ()
BEGIN
    DECLARE faturamento1,faturamento2,faturamento3 DOUBLE;
		SET @faturamento1=(SELECT (SUM(Faturamento_Produto)+SUM(Faturamento_Frete))-
			SUM(Custo_Produto)-SUM(Custo_Frete)-SUM(Custo_Mkt)-SUM(Desconto_Produto)-SUM(Desconto_Frete)
			FROM B2W.base_unica WHERE fk_departamento = 1);
		SET @faturamento2=(SELECT (SUM(Faturamento_Produto)+SUM(Faturamento_Frete))-
			SUM(Custo_Produto)-SUM(Custo_Frete)-SUM(Custo_Mkt)-SUM(Desconto_Produto)-SUM(Desconto_Frete)
			FROM B2W.base_unica WHERE fk_departamento = 2);
		SET @faturamento3=(SELECT (SUM(Faturamento_Produto)+SUM(Faturamento_Frete))-
			SUM(Custo_Produto)-SUM(Custo_Frete)-SUM(Custo_Mkt)-SUM(Desconto_Produto)-SUM(Desconto_Frete)
			FROM B2W.base_unica WHERE fk_departamento = 3);
		SELECT @faturamento1 AS `Bola de Gude`,@faturamento2 AS `Cadeiras`,@faturamento3 AS `Maquina Fotografica`;
END $$
DELIMITER ;
CALL departamento_maior_lucro()